...
export async function convert(
  inputPath,
  { theme: themeName = "claude", output, scale = 3, onProgress } = {},
) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    for (let j = 0; j < px.length; j += 4) {
      const factor =
        1 - (0.299 * px[j] + 0.587 * px[j + 1] + 0.114 * px[j + 2]) / 255;
    ...
    outDoc
      .addPage([vp.width, vp.height])
      .drawImage(img, { x: 0, y: 0, width: vp.width, height: vp.height });
